...
public class GeminiService
{
    ...
    public async Task<string> ProcessPromptAsync(string userMessage)
    {
        ...
        {
            await Task.Delay(5000); // Symulacja pracy AI
            return $"[AI Mock Response]\nOtrzymałem Twoją wiadomość: \"{userMessage}\".\n\n" +
                   ...
                   $"została zweryfikowana pomyślnie. Dodaj klucz API Gemini w appsettings.development.json, " +
                   $"aby zobaczyć prawdziwe odpowiedzi generowane przez model AI.";
        }
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            throw new Exception($"BŁĄD GOOGLE API: {errorBody}");
        }
